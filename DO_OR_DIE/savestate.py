...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.edge_locations = [(x, 14 + x) for x in range(14)] + [(14 + x, 27 - x) for x in range(14)]
    ...
    def left_right(self, game_state):
        ...
        demo_locs = [[2, 11], [25, 11]]
        support_locs = [[5,10],[22,10]]
        scout_locs = [[15, 1],[12, 1]]
        ...
        if not left_upgraded_turret_found:
            game_state.attempt_spawn(DEMOLISHER, demo_locs[0], 3)
            game_state.attempt_spawn(SUPPORT, support_locs[0], 1)
            game_state.attempt_spawn(SCOUT, scout_locs[0], 1000)
            game_state.attempt_upgrade(support_locs[0])
        
        elif not right_upgraded_turret_found:
            game_state.attempt_spawn(DEMOLISHER, demo_locs[1], 3)
            game_state.attempt_spawn(SUPPORT, support_locs[1], 1)
            game_state.attempt_spawn(SCOUT, scout_locs[1], 1000)
            game_state.attempt_upgrade(support_locs[1])

        else:
            tup = self.least_damage_spawn_location(game_state, demo_locs)
            if not tup: i = 0
            else: 
                if tup[0] == demo_locs[0]:
                    i = 0
                else:
                    i = 1

            game_state.attempt_spawn(DEMOLISHER, demo_locs[i], 3)
            game_state.attempt_spawn(SUPPORT, support_locs[i], 1)
            game_state.attempt_spawn(SCOUT, scout_locs[i], 1000)
            game_state.attempt_upgrade(support_locs[i])
    ...
    def layer1(self, game_state):
        ...
        unit_locations = [[TURRET, 13,13], [TURRET, 4,13],[TURRET, 5,13],[TURRET, 22,13], [TURRET, 23,13], [TURRET, 14,13], [SUPPORT, 5, 10], [SUPPORT, 9, 8], [SUPPORT, 12, 5], [SUPPORT, 15, 5], [SUPPORT, 18, 8], [SUPPORT, 22, 10]]
    ...
    def least_damage_spawn_location(self, game_state, location_options):
        ...
        for location in location_options:
            ...
            if path and any(path):
                illegal = True
                for path_location in path:
                    if path_location in self.edge_locations: illegal = False
                    for attacker in game_state.get_attackers(path_location, 0):
                        if attacker.unit_type == TURRET:
                            damage += attacker.damage_i
            if illegal: damages.append(600)
            else: damages.append(damage)
